class UIManager {
    ...
    updateHUD() {
        ...
        // Update target health bars and threat info
        Object.entries(this.game.targetManager.targets).forEach(([key, target]) => {
            ...
            // Update facility threat status
            this.updateFacilityThreatStatus(key, target);
        });
    ...
    updateFacilityThreatStatus(facilityKey, target) {
        // Create or update threat status display
        const threatIndicator = document.getElementById(`${facilityKey}Threat`);
        if (threatIndicator) {
            if (target.missiles > 0) {
                threatIndicator.textContent = `🚀${target.missiles} (${target.threatLevel})`;
                threatIndicator.style.color = this.getThreatColor(target.threatLevel);
                
                if (target.attackCooldown > 0) {
                    threatIndicator.style.opacity = '0.5';
                } else {
                    threatIndicator.style.opacity = '1.0';
                    threatIndicator.style.fontWeight = 'bold';
                }
            } else {
                threatIndicator.textContent = '🚀0 (EMPTY)';
                threatIndicator.style.color = '#666666';
                threatIndicator.style.opacity = '0.5';
            }
        }
    }
